...
header = 
    row [width fill, paddingXY 16 8, Background.color (rgb255 50 50 50), Font.color (rgb255 245 245 245), spacing 8] 
    [ Input.button 
    [Border.width 1,
    Border.rounded 4,
    paddingXY 8 4] 
    ...
    Border.rounded 4,
    paddingXY 8 4] 
    {label  = text "Convert to Json", onPress = Just ConvertButtonPressed} ]
...
content model = 
        case model.file of 
            ...
            Loaded loadState ->
                column [ padding 8, spacing 16, width fill, height fill, scrollbars ] 
                [
...
renderOutput loadState = 
    ...
    in
    row [width fill, height fill, spacing 32] 
...
renderParsedCSV headers previewRows =
    section "Parsed CSV"
        (table [] { columns = List.indexedMap createHeader headers, 
                                data = previewRows })

createHeader: Int -> String -> Column String Msg 
createHeader index name = 
    {header = 
        el [paddingXY 8 4] (text name), 
        width = shrink, 
        view = \rowString -> 
        el [paddingXY 8 2] 
            (rowString
...
createJsonObject headers stringRow= 
    E.object (List.map2 (\ name stringValue ->
...
renderRawInput previewRows = 
    section "First ten lines of raw input"
            (column [
                width fill, 
                height fill
                ]
...
update msg model =
        case msg of 
            ...
            FileContentLoaded str ->
                ...
                ({file = 
                Loaded 
                    {previewRows = List.take noOfPreviousRows rows, 
                    headers = 
                        List.head rows 
                            |> Maybe.map (String.split delimiter)
                            |> Maybe.withDefault []
            ...
            ConvertButtonPressed ->
                (model, 
                    case model.file of
                        ...
                        Loaded loadState ->
                            let
                                jsonString = buildJsonString loadState.headers loadState.allRows 
                            in
                                File.Download.string "csv2json.json" "application/json" jsonString)